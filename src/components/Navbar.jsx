  ...
  return (
    <>
      ...
      <div style={{ background: C.rust, padding: isMobile ? '8px 16px' : '9px 32px', display: 'flex', alignItems: 'center', justifyContent: isMobile ? 'center' : 'space-between', gap: 12 }}>
        {!isMobile && <span style={{ fontFamily: "'Special Elite', monospace", fontSize: 10, letterSpacing: '0.2em', textTransform: 'uppercase', color: 'rgba(245,240,232,0.6)' }}>⚠ Protest deadline</span>}
        <span style={{ fontFamily: "'Caveat', cursive", fontSize: isMobile ? 17 : 22, fontWeight: 700, color: C.cream }}>
          {isMobile ? '⚠ Protest deadline: May 15, 2026' : 'May 15, 2026'}
        </span>
        {!isMobile && <span style={{ fontFamily: "'Special Elite', monospace", fontSize: 10, color: 'rgba(245,240,232,0.45)', letterSpacing: '0.05em' }}>Or 30 days from your notice — whichever is later</span>}
      </div>
 
      {/* Nav bar */}
      <nav style={{ position: 'sticky', top: 0, zIndex: 50, background: scrolled ? 'rgba(245,240,232,0.97)' : C.cream, borderBottom: `2px solid ${C.brown}`, backdropFilter: scrolled ? 'blur(8px)' : 'none', transition: 'all 0.2s ease' }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: `${isMobile ? '12px' : '14px'} ${px}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          {!isMobile && !isTablet && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 32 }}>
              {[['#how', 'How It Works'], ['#pricing', 'Pricing']].map(([h, l]) => (
                <a key={h} href={h} className="nav-link" style={{ fontFamily: "'Special Elite', monospace", fontSize: 12, letterSpacing: '0.12em', textTransform: 'uppercase', color: C.faded, textDecoration: 'none', transition: 'color 0.15s' }}>{l}</a>
              ))}
          ...
          {isTablet && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
              <Link to="/intake" className="nav-cta" style={{ fontFamily: "'Special Elite', monospace", fontSize: 10, letterSpacing: '0.12em', textTransform: 'uppercase', border: `2px solid ${C.rust}`, color: C.rust, padding: '7px 16px', textDecoration: 'none', background: 'transparent', transition: 'all 0.15s' }}>Start Free →</Link>
              <button className={`hamburger ${menuOpen ? 'open' : ''}`} onClick={() => setMenuOpen(o => !o)} aria-label="Menu"><span /><span /><span /></button>
            </div>
          )}
        ...
        {(isMobile || isTablet) && (
          <div className={`mobile-menu ${menuOpen ? 'open' : 'closed'}`} style={{ borderTop: '1px dashed rgba(60,30,8,0.2)', background: C.cream }}>
            <div style={{ padding: '16px', display: 'flex', flexDirection: 'column' }}>
              {[['#how', 'How It Works'], ['#pricing', 'Pricing']].map(([h, l]) => (
                <a key={h} href={h} onClick={() => setMenuOpen(false)} style={{ fontFamily: "'Special Elite', monospace", fontSize: 13, letterSpacing: '0.12em', textTransform: 'uppercase', color: C.faded, textDecoration: 'none', padding: '14px 0', borderBottom: '1px dashed rgba(60,30,8,0.15)', display: 'block' }}>{l}</a>
              ))}
