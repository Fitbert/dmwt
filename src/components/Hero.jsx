...
export default function Hero() {
  ...
  return (
    <section style={{ background: C.cream, position: 'relative', overflow: 'hidden', minHeight: isMobile ? 'auto' : '90vh', display: 'flex', alignItems: 'center' }}>
      ...
      {/* Small accent stars */}
      <div style={{ position: 'absolute', top: '10%', left: '3%',  fontSize: 24, color: C.rust, opacity: 0.12, pointerEvents: 'none' }}>★</div>
      <div style={{ position: 'absolute', top: '80%', left: '5%',  fontSize: 16, color: C.rust, opacity: 0.08, pointerEvents: 'none' }}>★</div>
      <div style={{ position: 'absolute', top: '20%', right: '30%',fontSize: 18, color: C.rust, opacity: 0.07, pointerEvents: 'none' }}>★</div>
 
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: `${isMobile ? '56px' : '88px'} ${px}`, width: '100%', position: 'relative', zIndex: 1 }}>
        <div className="hero-grid">
          ...
          <div>
            ...
            <motion.p {...fadeUp(0.32)} style={{ fontFamily: "'Special Elite', monospace", fontSize: isMobile ? 12 : 13, lineHeight: 1.9, color: C.ink, maxWidth: 430, marginBottom: 32, opacity: 0.8 }}>
              We pull comparable sales, run the equity analysis, and put together everything your appraisal review board needs to see — formatted proper, ready to file. You show up. You fight back.
            </motion.p>
          ...
          >
            ...
            <div style={{ background: '#fffef9', border: '1.5px solid rgba(60,30,8,0.3)', padding: '28px 26px', position: 'relative' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 16 }}>
                <div style={{ border: '2.5px solid rgba(122,26,26,0.7)', padding: '3px 10px', fontFamily: "'Special Elite', monospace", fontSize: 9, letterSpacing: '0.22em', textTransform: 'uppercase', color: 'rgba(122,26,26,0.7)', transform: 'rotate(-4deg)', fontWeight: 700 }}>Overvalued</div>
                <div style={{ border: '2.5px solid rgba(26,46,90,0.6)',  padding: '3px 10px', fontFamily: "'Special Elite', monospace", fontSize: 9, letterSpacing: '0.22em', textTransform: 'uppercase', color: 'rgba(26,46,90,0.6)',  transform: 'rotate(3deg)',  fontWeight: 700 }}>Ready to File</div>
              </div>
              ...
              {[
                ...
                { key: 'Packet status', val: 'Ready ✓',  color: '#1a5c2a' },
              ].map(({ key, val, color }) => (
